...
- `CLAUDE_PREFLIGHT.md` — 27 numbered rules covering credential handling (Rules 21–22), credential-leak prevention in logs (URL-based auth + httpx logging), execution-context labelling for every command block (Rule 16), placeholder/assumption auditing (Rule 20), fail-loud error handling (Rule 18), verification-before-conclusion (Rule 14), bash-mount-staleness on Windows-side files (Rule 24), session-anchor verification at the start of every chat — date/time, working directory `C:\trading\LLM model`, workstation "Godzilla" (Rule 25), the hard partition between the LLM-model fork (Godzilla) and the gap-and-go fork (old laptop + VPS at `5.161.199.155`) — no cross-fork SSH, DB access, repo ops, or operational context (Rule 26), and durability verification at session-end — every session wrap must verify `git push` landed on the fork's own remote before claiming work is "shipped" or "ARMED" (Rule 27). These are corrections to prior failures; treat them as hard requirements, not style suggestions.
...
"Compile + tests pass" is not "session done." Pass-on-disk is not the same as pushed-to-origin. Before writing a session summary or generating a handoff prompt for a future session, run from PowerShell on the workstation (NOT bash, per Rule 24): (1) `git status` — every modified/untracked project-artifact file (code, tests, docs, configs) is a blocker until acknowledged; (2) `git add <files>` + `git commit -m "<descriptive message>"` — message must name what shipped; (3) `git push` to the fork's own remote — on Godzilla this is whatever local label points at `https://github.com/NZ1979/trading-model-llm.git` (verify with `git remote -v` first; on the current Godzilla clone the label is `origin`, but a future re-clone may name it differently — push by intent, which is "the LLM-fork's remote URL", not by a hardcoded local label). Gap-and-go remote symmetrically from gap-and-go sessions; cross-pushing between forks is a Rule 26 violation. (4) re-run `git status` to confirm "working tree clean" AND "Your branch is up to date with '<remote>/main'" (where `<remote>` is whatever label was used in step 3). The session summary must include a literal **`Committed and pushed: <SHA>`** line. Words like "ARMED," "shipped," "delivered," "complete," "ready," "deployed," or "done" must not appear in a wrap until all four checks pass — until the push lands, the accurate framing is *"code on disk, tests passing, NOT YET committed."* Specific trap (2026-05-13): a session shipped ~950 lines of new code, including entire new modules (`strategy/llm/policy.py` ~900 lines, `strategy/llm/context_builder.py` ~420 lines, `analysis/regime.py` + `analysis/regime_data.py`, plus 5 new test files for ~155 tests), ran `py_compile` and `pytest -q` (231 passed) repeatedly, and wrapped as "Code is ARMED" with a tomorrow-handoff prompt. **Zero git operations occurred end-to-end.** Next morning's session opened with 9 modified + 16 untracked files. Damage was zero (disk held) but trajectory was toward losing a full day on a single hiccup. `py_compile` and `pytest` verify behavior of files on disk; they say nothing about git state. If the user explicitly requests something stay uncommitted (WIP experiment, rollback-pending config), capture that exception in the summary: "**Per user request, NOT committed:** `<files>` — `<reason>`." Anything else is the failure mode this rule catches.
