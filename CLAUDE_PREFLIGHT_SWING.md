# Claude Pre-Flight Checklist — LLM Swing Model

This file is the LLM Swing Model's operational rulebook. It supersedes `CLAUDE_PREFLIGHT.md` from the legacy intraday project (retained read-only at `C:\trading\LLM model\` as historical reference, per Rule 26).

This file exists because Claude wasted hours of my time during the legacy intraday project with instructions that were stale, untested, or confidently wrong. Future Claude sessions on the swing model: read this before giving me ANY operational instruction. Apply every rule that's relevant. No exceptions.

## Notes on this version (created 2026-05-17 during scrub-and-migrate)

Most rules carry over verbatim from `CLAUDE_PREFLIGHT.md`. Specific changes:

- **Rules 9 and 10**: removed. Rule 9 was gap-and-go VPS operational commands (Hetzner at 5.161.199.155, `/opt/trader/app/`, `trader.service`). Rule 10 was the gap-and-go fork's deployment-status snapshot as of 2026-04-29. Neither applies to the swing model, which has no current deployment. Both slots are reserved for re-issuance once the swing model's deployment target is decided (Phase R5/R6 of `docs/ROADMAP.md`).
- **Rule 16**: example execution-context labels updated to remove the trader-prod / VPS console entries and add Godzilla-centric ones. Trap narrative preserved verbatim as historical record.
- **Rule 25**: cross-references updated. The legacy `LLM_MODEL_CHARTER.md` and intraday `CLAUDE.md` references now point to `PROJECT_CHARTER.md`, `docs/HARDWARE_PLATFORM.md`, and `docs/ARCHITECTURE.md`. Working directory anchor updated from `C:\trading\LLM model` to `C:\trading\LLM_SWING_MODEL`.
- **Rule 26**: fork identification updated to use "LLM Swing Model fork" terminology. Example file paths updated from legacy intraday script names to swing-model module names. The `C:\trading\LLM model\` path is added to the read-only reference list (sharing the same partition discipline as the gap-and-go fork). Tripwire list and "specific trap" historical narrative preserved.
- **Historical trap narratives** ("Specific trap already fallen into" sections) are preserved verbatim throughout. They reference the legacy intraday work; the principle each rule encodes still applies to swing-model work.
...
## Rule 9: [Reserved — see note below]

**Note (2026-05-17, scrub to LLM_SWING_MODEL):**
The original Rule 9 contained operational commands for the gap-and-go fork's Hetzner VPS deployment (SSH instructions, `systemctl` commands, `journalctl` commands, `scp` patterns targeting `5.161.199.155`). The LLM Swing Model has not yet established its own deployment target. That decision is part of Phase R6 of the roadmap in `docs/ROADMAP.md` (options: Godzilla as primary runtime, separate VPS, or hybrid).

This rule slot is reserved for re-issuance once the swing model's deployment target is known. Until then, no live-deployment operational guidance exists for this project. Any "let's deploy this to the VPS" recommendation that surfaces in chat before Phase R6 lands is a Rule 26 partition violation against the legacy gap-and-go infrastructure, not a swing-model action.
...
If you (Claude in a future session) see this file in the project, you've read it. Now apply all rules (currently 1-27, with 9 and 10 reserved) to whatever the user asks. If the user reports an instruction failed, check whether you violated one of these rules first. Apologize concretely (which rule, what specific oversight). Don't apologize abstractly ("sorry for the confusion") and move on to a new mistake.
...
Swing-model execution contexts (active):

- **"In a normal PowerShell window on your Windows machine (Godzilla):"** — for local PowerShell on the workstation. Windows file paths, `curl.exe`, git operations against the local repo, `python`, `pip`, `Get-Content`, `Get-ChildItem`, etc.
- **"In the Godzilla `.venv` Python REPL (PowerShell with venv activated):"** — when running Python interactively after `.\.venv\Scripts\Activate.ps1`
- **"In LM Studio's local server console on Godzilla:"** — for managing the Qwen 3.6-27B inference server (when set up in Phase R2). LM Studio's UI tab, not a separate shell.
...
Reserved for future deployment contexts (Phase R6+):
- Remote shell labels for the swing model's deployment target will be added here once the target is decided (Godzilla as primary, or separate VPS).

**Legacy contexts NOT applicable to this project (Rule 26 partition):**
- "In the in-browser console connected to `root@trader-prod`:" — gap-and-go fork's VPS console
- "In an SSH session to the VPS (run from PowerShell):" with `trader-prod` prompt — gap-and-go fork's SSH session

These labels appear in older session transcripts. If they show up in a swing-model session, that's a partition violation per Rule 26 and should be flagged before sending.

If switching contexts within a single response, label EACH block — never assume the previous label still applies.

**Specific trap already fallen into (historical, legacy intraday project):** During the 2026-05-02 deploy, Claude alternated between PowerShell commands and in-browser console commands without consistent labeling. The user repeatedly had to ask "where do I type this?" — once explicitly: "do i type those prompts into the in-browser console?" Each ambiguity cost a round-trip and compounded hours of frustration.
...
**Specific traps already fallen into in the legacy intraday project:**
...
**Project-specific corollary (preserved from legacy preflight):** The Hetzner VPS at `5.161.199.155` is NOT the swing model's deployment. It runs the gap-and-go fork (`llm.enabled: false`) on account `PA3REQ1LMPKO`. The Large Cap account `PA3QAZ941NFN` is reserved for when the LLM Swing Model eventually deploys, on Godzilla as runtime or a separate VPS to be provisioned. The two must not co-exist on a single `trader.service`. Verify which fork's code runs where before recommending any infrastructure action. See Rule 26 for the full partition discipline.
...
The Cowork session exposes two file-access surfaces for files under the swing-model project root: the file tools (`Read`/`Write`/`Edit`) and the bash sandbox mount at `/sessions/<session-id>/mnt/<folder>/`. The system prompt says these are the same files. In practice they can drift: the file tools' view can be hours ahead of what the bash mount serves, even after a successful `Edit` call that reports "file updated successfully." Verifying file content via the bash mount is NOT equivalent to verifying it on the actual Windows disk that git, the trader, and any deploy tool will read.

**The rule, hard:**

1. **For Windows-side files (anything under the swing-model project root on disk), the bash mount is best-effort cache, not ground truth.** Treat its `cat`/`wc -l`/`tail`/`md5sum` output as informational only when reconciling with `Edit`/`Read`.
...
5. **The authoritative verification for Windows-side files runs from PowerShell on the user's workstation, not from any tool in the Cowork session.** Use:
   ```powershell
   # In a normal PowerShell window on your Windows machine (Godzilla), from C:\trading\LLM_SWING_MODEL\:
...
2. **Working directory is `C:\trading\LLM_SWING_MODEL`.** This is the swing-model project root and the only path under which this `CLAUDE_PREFLIGHT_SWING.md` applies. All file paths, git operations, and PowerShell command blocks anchor here. If the user appears to be operating from a different path, surface the mismatch before continuing — the rule sets and the running project are not the same. Specific paths that are NOT this project:
   - `C:\trading\LLM model\` — the legacy intraday project, retained read-only as historical reference per Rule 26. Any write or git operation against this path is a partition violation.
   - `C:\trading\trading-platform\` — upstream base codebase, if present.
   - `/opt/trader/app/` on the VPS — gap-and-go fork's production directory.

3. **Workstation is "Godzilla".** Godzilla is the user's local workstation, intended to host the local Qwen 3.6-27B inference path via LM Studio per `docs/HARDWARE_PLATFORM.md` and the catalyst-driven multi-day architecture defined in `PROJECT_CHARTER.md` and `docs/ARCHITECTURE.md`. Godzilla is distinct from:
   ...
   Recommendations involving local hardware (GPU, VRAM, LM Studio config, CUDA, local model file paths, `.venv` activation) apply to Godzilla. Recommendations involving systemd, journalctl, `/opt/trader/app/`, or `/etc/trading-platform/env` apply to the gap-and-go VPS and are out of scope for swing-model sessions per Rule 26. Never conflate them.

State all three verifications inline in the first substantive response of the session, e.g.:

> Verified: Wed 2026-05-13 09:21 EDT; working in `C:\trading\LLM_SWING_MODEL`; workstation Godzilla.

**How to apply:**

- The first tool calls of any new chat with operational intent are `date && TZ=America/New_York date` plus, when ambiguous, a quick directory/file existence check to confirm we're talking about the swing-model project root.
- If the user references a path outside `C:\trading\LLM_SWING_MODEL`, pause and confirm which codebase the action targets before issuing commands.
- For any recommendation involving local model execution, LM Studio, GPU memory, local CUDA, or workstation-side Python: assume Godzilla. For systemd / journalctl / VPS paths: that's the gap-and-go Hetzner box, out of scope.
- Cross-reference Rule 20: the Hetzner VPS is NOT the swing model's deployment. Recommending a swing-model action on the VPS without an explicit migration plan is the trap Rule 20 was written to catch.

## Rule 26: Hard partition between LLM Swing Model (Godzilla) and gap-and-go (old laptop + VPS), plus read-only discipline for the legacy intraday archive

This project (LLM Swing Model) and the previously-active forks operate on separate machines, separate Cowork sessions, and separate Alpaca accounts. They do not share operational threads, not even read-only ones for the live fork. Rules 20 and 25 stated the separation at a planning level; this rule makes it operational and enforceable.

**The three relevant codebase locations:**

- **LLM Swing Model fork (this project)**: Lives at `C:\trading\LLM_SWING_MODEL\` on workstation **Godzilla**. Multi-day catalyst-driven swing trading per `docs/STRATEGY_RESET_2026-05-16.md`. Future home of Qwen 3.6-27B tier-1 via LM Studio. Will eventually deploy to Alpaca paper account `PA3QAZ941NFN` (Large Cap) on infrastructure to be provisioned (Godzilla as runtime, or a separate VPS — TBD in Phase R6).
- **Legacy intraday archive**: Lives at `C:\trading\LLM model\` on Godzilla. The pre-scrub state of the LLM-fork intraday work, parked on 2026-05-17 because the architecture was non-viable per `docs/STRATEGY_RESET_2026-05-16.md`. **Read-only reference**. Not for active development. Not to be `cd`-ed into for write operations. May be inspected for historical context, lifted code patterns (with attribution), or recovery of specific lost files via copy.
- **Gap-and-go fork (separate project entirely)**: Lives on the user's **old laptop** as its development root, and is deployed to the **Hetzner VPS at `5.161.199.155`** (`/opt/trader/app/`, `trader.service`, account `PA3REQ1LMPKO`). Currently running paper trades; `llm.enabled: false`. Maintained independently from Godzilla.

**The rule, hard:**

From any session anchored in `C:\trading\LLM_SWING_MODEL\` on Godzilla (per Rule 25):
...
3. Do NOT commit, push, pull, or fetch against the gap-and-go fork's repository. The swing-model fork uses its own repository remote (URL to be established at first commit; see Rule 27 for push discipline once it's known); operations stay on that remote only.
4. Do NOT reference gap-and-go operational state (decisions, orders, account balances, journalctl output, systemd status) as context for swing-model work. Even quoting it for "baseline comparison" is a partition violation — use local test data, replay fixtures, or deliberately-curated sanitized snapshots if realistic data is needed.
5. Do NOT recommend operational steps that take a swing-model script (anything under `C:\trading\LLM_SWING_MODEL\scripts\` that imports from `research.*`, `decision.*`, `knowledge_base.*`, `measurement.*`, or other swing-specific packages) and run it against the VPS's data or codebase. Swing-model scripts import from modules that do not exist in the gap-and-go fork; running them touches swing-model code paths against gap-and-go data, which is exactly the contamination this rule prevents.
6. Do NOT write to, edit, or `git` any file under `C:\trading\LLM model\` (the legacy intraday archive). Read access for historical reference is allowed; write access is forbidden. If you find yourself wanting to "fix" something there, the right move is to copy the file forward to `C:\trading\LLM_SWING_MODEL\` and edit the copy, leaving the archive frozen.

Symmetric prohibitions apply from gap-and-go sessions and from any session that accidentally lands in the legacy archive:

7. From any session anchored in the gap-and-go fork on the old laptop: do NOT touch `C:\trading\LLM_SWING_MODEL\`, Godzilla, or the swing-model repo. Do NOT pull swing-model commits into the gap-and-go fork. Operational ops of the gap-and-go fork happen on the old laptop, in the gap-and-go fork's repo, in separate Cowork sessions on that machine.
8. From any session that accidentally lands in `C:\trading\LLM model\` (the legacy intraday archive): treat as a misanchor per Rule 25. The legacy archive is not an active development location. Re-anchor to `C:\trading\LLM_SWING_MODEL\` before continuing.

**The only legitimate crossing points** are:

- The eventual migration when the swing model deploys to its own account (`PA3QAZ941NFN`) on its own infrastructure. That deploy is a planned, documented event with its own checklist, its own pre-flight audit, and its own atomic cutover. It happens in a dedicated session, NOT as a side effect of swing-model development work.
- Read-only inspection of `C:\trading\LLM model\` for historical reference. Allowed; not encouraged. Lift code forward by copying to the swing-model project; do not edit in place.

**Detection and response:**

Treat the following strings as **TRIPWIRES** inside a swing-model session — their appearance in a command, file path, recommendation, or even an explanatory paragraph means a partition check is required before sending: `5.161.199.155`, `/opt/trader/app/`, `hetzner_trader`, `PA3REQ1LMPKO`, `trader.service`, `trader-prod`, and (for the read-only archive) `C:\trading\LLM model\` appearing in any write context (`git add`, `Edit`, `Write`, redirect into a file, etc.).

If a request inside a swing-model session implies touching the gap-and-go side — phrases like "backfill from production," "check the VPS logs," "use the prod decisions table as ground truth," "ssh to the trader server" — STOP and name the partition violation before doing anything. The right answer is almost always "operate within Godzilla on the swing-model fork alone." If the user genuinely needs cross-fork data, that becomes its own scoped task with its own constraints, requested deliberately, not pulled in as ambient context.

**Specific trap already fallen into (2026-05-13, legacy intraday session):** A session anchored at `C:\trading\LLM model\` on Godzilla repeatedly crossed the partition: (1) had the user SSH from Godzilla into the VPS to inspect `/opt/trader/app/trading.db`; (2) was about to recommend copying that DB down to Godzilla and running `scripts/backfill_shadow_outcomes.py` — an LLM-fork-specific script that imports `strategy.llm.metrics` — against the gap-and-go fork's decisions, which would have applied LLM-fork code paths to gap-and-go data and produced `shadow_outcomes` rows that conflate the two; (3) was treating gap-and-go decision counts as "useful baseline data for the LLM model," when they're separate-codebase, separate-account data that has no business being analyzed from a swing-model session. The user caught the third violation explicitly: "is the LLM model actually trading, or just running data in the background? confirm this is not the gap-and-go model." Damage was zero (read-only queries, no writes, no LLM-fork scripts executed against VPS), but the trajectory was clearly toward contamination. The fix is this rule.

**How to apply:**

- Every session opens with Rule 25's anchor verification: date/time, working directory `C:\trading\LLM_SWING_MODEL`, workstation Godzilla. If the working directory is anywhere outside `C:\trading\LLM_SWING_MODEL`, this rule's session-side prohibitions do not apply (you may be in a gap-and-go session instead, or in the read-only archive — apply the symmetric prohibitions there).
- For swing-model development that needs realistic data, use SYNTHESIZED fixtures or a deliberately-curated local SQLite DB that lives only inside the swing-model repo (e.g., `tests/fixtures/` or a small committed sample DB). Do NOT pull from the gap-and-go production DB. Lifting data from `C:\trading\LLM model\trading.db` (the legacy archive) is allowed as a one-time read for narrowly-scoped purposes (e.g., extracting a historical sentiment series for a specific ticker); document the lift in a session note.
- When the swing model is ready to deploy, that's a planned cutover with its own session, its own account credentials (`PA3QAZ941NFN`), its own infrastructure decision (Godzilla as runtime vs a fresh VPS), and its own pre-flight gates. Not a "let's just borrow the existing VPS for a bit" move.
- This rule supersedes any earlier conversational pattern in which a swing-model session has discussed gap-and-go state. If you (Claude) find yourself reaching for VPS context inside a swing-model session, that's the signal to stop and re-anchor.
...
3. `git push` — push to the swing-model fork's own remote. Verify with `git remote -v` first; push by intent (the URL of the swing-model fork's remote), not by a hardcoded local label. Gap-and-go remote symmetrically from gap-and-go sessions — never cross-push per Rule 26. The remote URL for the swing-model fork is established at first push and tracked from there.
4. Re-run `git status` — must now show "Your branch is up to date with 'origin/main'" (or whatever local label points at the swing-model remote) AND "working tree clean".
...
- Apply symmetrically to both forks. Gap-and-go session wraps must verify push to the gap-and-go remote URL; swing-model session wraps must verify push to the swing-model fork's remote URL. Run `git remote -v` once to confirm which local label points at the intended URL; never hardcode the local label, push by intent. Cross-pushing between forks (the gap-and-go remote URL from a swing-model session, or vice versa) is a Rule 26 violation; same-fork pushing is mandatory per this rule.
